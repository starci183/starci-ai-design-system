...
> Nguyên tắc xuyên-suốt (họ `principles/*`). Rút kernel cross-cutting từ  §1 (viết riêng cho surface landing) — áp dụng chung cho MỌI surface, không chỉ marketing.
...
> Đã áp:  §1-2 (grounded-in-data cho landing: field null → không phụ thuộc; reframe taxonomy theo content thật; kill filter trỏ bucket rỗng).
