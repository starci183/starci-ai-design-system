...
- **KHÔNG có list đủ nuôi → KHÔNG rail.** "2-3 mode toggle" KHÔNG phải list → đừng nhồi vào rail (sẽ rỗng). Đính chính trực tiếp luật cũ (*"2 item thôi KHÔNG đủ nuôi rail"*).
...
|---|---|---|
| 1 trục nav + **list dài** (module/bài, deck, topic, category) | **2-pane: rail trái + pane phải** | — |
| Nav chỉ **2-3 mode/option**, không sub-list | **TabsCard/SegmentedControl đầu pane** + 1 cột | — |
...
| 1 cột đọc / setup / form focused (1 task) | **centered `max-w-3xl mx-auto`, no rail** | — |

## Đặt rail ở ĐÂU (nếu dùng)
- **Route trong LearnShell** → rail = `LearnShell.leftRail` (layout-level), state qua **URL** (rail ≠ pane cùng cây).
- **Route STANDALONE** (không shell) → **page-internal 2-pane** (`ResizableRail` + content tự `p-6`).

## Mobile (STRICT)
- Rail dọc cạnh-trái CHỈ hợp `lg+`. Màn hẹp → rail `hidden lg:flex` + **fold thành hàng chip cuộn ngang** (mode chips + topic chips) TRÊN đầu pane, đọc cùng URL state.
...
## Liên quan
- [[when-drawer]] (giấu phụ sau drawer) · [[solving-surface-fullbleed-no-course-rails]] / [[fullbleed-canvas-no-chrome-and-orient-zoom]] (full-bleed bỏ rail) · [[elements/sidebar]] (block rail).
