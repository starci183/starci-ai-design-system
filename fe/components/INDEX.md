...
| `blocks/grading/{GradeCreditCaption,GradeModelDropdown,GradingByline,SelfHostGpuMark}` | Đã có rải trong  + ; cần gom 1 `grading.md`. |
...
| `blocks/cards/{ContinueCard,MediaCard,PricingCard,PressableCard,CourseCard,CourseCardSkeleton}` | Rải trong `card.md` (PressableCard nhắc ở §3c/gotcha, CourseCard ở gotcha-render); `ContinueCard`/`MediaCard`/`PricingCard` chưa có mục riêng. |
