...
- **Title dài trong rail hẹp = title FULL-WIDTH 1 dòng + `truncate` + tooltip (`title` attr); meta (count "n/m", bar) xuống DÒNG 2 dưới title, caret canh giữa phải.** ĐỪNG để count/caret cùng hàng cướp width title.
...
- **Full-bleed opt-out = ngoại lệ DUY NHẤT** (canvas tràn viền, vd mind-map) → truyền `fullBleed` để BỎ `p-6`. Mọi trang "đọc" khác đều padded.
- **Settings/profile shell = KHÔNG padding khung** (rail + content mỗi bên tự lo). **Content column** owns `mx-auto max-w-3xl p-6` (1 frame); **trang con render BARE** (`flex flex-col gap-6`) — đừng để shell-frame + child-frame chồng (double p-6/max-w).
...
- Page feed lớp-2 qua store `useRegisterNavbarBottomLayer(useMemo node)` (Navbar global). Strip lớp-2 (ProfileTabsBar/DashboardTabsBar) bỏ `sticky/border/bg` riêng.

## 6. Rail nội dung (learn ContentMap / OnThisPage) + ResizableRail
- Sticky `top-16`, full-height, vùng cuộn bọc **`ScrollShadow`** (fade mép, max-h) — KHÔNG overflow trần.
- Title rail dài → full-width 1 dòng + `truncate` + tooltip; meta/count xuống dòng 2.
...
- Rail nhiều block: ẩn theo MỤC ĐÍCH container, không ẩn cả rail vì 1 block rỗng.

## 6b. Rail docs-style ở đâu = TÙY route có LearnShell hay không (STRICT)
- **Route TRONG `/learn/*` (có LearnShell)** → rail = `LearnShell.leftRail` (rail ở LAYOUT), state qua URL.
- **Route STANDALONE top-level (KHÔNG có LearnShell, vd `/practice`)** → rail = **PAGE-INTERNAL 2-pane**: dựng ngay trong feature (`<div className="flex flex-col lg:flex-row">` chứa `ResizableRail` + content), **content TỰ khai `p-6`** (không LearnShell lo → đính chính: shell-sở-hữu-p6 chỉ đúng khi CÓ shell). Rail className mirror LearnShell (`hidden shrink-0 lg:sticky lg:top-16 lg:flex lg:h-[calc(100dvh-4rem)] lg:flex-col lg:self-start`).
- **Cả 2 kiểu:** state qua URL (`?view=`/`?domain=`, KHÔNG local `useState`) → rail + pane + mobile-chip-row cùng 1 nguồn; mode/topic = `ListBox` dùng `onSelectionChange` (KHÔNG `ListBox.Item onAction`); mobile fold rail thành CHIP-ROW cuộn ngang (`lg:hidden`). 1 surface = 1 đường route (đừng để `/practice` + `/learn/practice` trùng).
...
- **Full-bleed route:** route render editor BARE (không wrapper padded); editor tự sở hữu shell (đính chính: shell-sở-hữu-p6 chỉ đúng khi có LearnShell; editor standalone tự lo).

## Liên quan
- [[when-rail]] · [[elements/alert]] (funnel-CTA) · [[elements/label]] (section label).
